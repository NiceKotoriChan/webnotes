...
Base path `/api`，**19 条路由，方法只有 `GET` 与 `POST`**。响应一律 JSON，错误形如 `{ "error": "<msg>" }`。无鉴权、无版本前缀。
...
| GET / HEAD | `/icons/mdi_rules_custom.json`  | 自定义图标规则  |

图标集全量一次性下发，由前端缓存；后端持本地副本，启动时缺失才下载，之后每 24h 后台检查。响应带 `ETag` 与 `Cache-Control: no-cache`，命中 `If-None-Match` 回 304；规则文件同理，但每次请求现读盘，改完立刻生效。**规则文件不在就回 404**，前端按「只有默认规则」处理。

**为什么有规则文件**：mdi 的图标名很难从笔记标题直接匹配出来，所以要有一份规则把标题映射到图标名 —— `mdi_rules_default.json` 是内置的默认规则，`mdi_rules_custom.json` 供自定义。两份文件同构，**后端只负责把文件发出去** —— 不合并两份规则、不解析、不校验、也不写盘；改动规则由前端决定（匹配在 `icon` 为 null 时做，见 [schema.md](schema.md)）。
